...
public class ChatUtil {
    ...
    public static void errno(CommandSender sender, ChatTypes type) {
        switch (type) {
            ...
            case PLAYER_NEVER_PLAYED ->
                    sender.sendMessage(formatWithPrefix(DATA_MANAGER.getConfig().getString("messages.player-never-joined")));
        }
    }
    ...
    public static String formatWithPrefix(String msg) {
        msg = ChatColor.translateAlternateColorCodes('&', DATA_MANAGER.getConfig().getString("prefix") + " " + msg);
        if (IS_NEWER_VERSION)
            msg = HexUtil.hex(msg);
        return msg;
    }
