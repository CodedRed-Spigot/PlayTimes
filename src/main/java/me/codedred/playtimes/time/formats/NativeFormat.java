...
public class NativeFormat implements Timings {
  ...
  private String formatSeconds(long seconds) {
    return (
      seconds +
      (seconds == 1 ? TimeConstants.getSeconds() : TimeConstants.getSecond())
    );
  }
  ...
  private String formatMinutes(int minutes, int secondsLeft) {
    String format =
      minutes +
      (minutes == 1 ? TimeConstants.getMinute() : TimeConstants.getMinutes());
  ...
  private String formatHours(int hours, int minutesLeft, int secondsLeft) {
    String format =
      hours + (hours == 1 ? TimeConstants.getHour() : TimeConstants.getHours());
    if (minutesLeft >= 1) format +=
      formatMinutes(minutesLeft, secondsLeft); else if (
      secondsLeft > 0
  ...
  private String formatDays(int days, int minutesLeft, int secondsLeft) {
    String format =
      days + (days == 1 ? TimeConstants.getDay() : TimeConstants.getDays());
    if (minutesLeft >= 60) {
